...
// --- Helper Functions specific to chat-ai-assistant ---
...
serve(async (req) => {
  ...
  try {
    ...
    // Semantic Retrieval (search_document_embeddings)
    const queryTextForSemanticSearch =
      documentSummaryForSemanticSearch && documentSummaryForSemanticSearch.trim() !== ""
        ? documentSummaryForSemanticSearch
        : message;

    const queryEmbedding = await createEmbedding(openAIKeyResult.key, queryTextForSemanticSearch, EMBEDDING_MODEL);
    ...
    // Assemble and Truncate Prompt
    const modelTokenLimit = getModelTokenLimit(CHAT_MODEL);

    let finalPrompt = rebuildPrompt(
    ...
    let totalPromptTokens = countTokens(CHAT_SYSTEM_PROMPT + finalPrompt);
    ...
    if (totalPromptTokens > modelTokenLimit) {
      ...
      if (
        ...
        totalPromptTokens = countTokens(CHAT_SYSTEM_PROMPT + finalPrompt);
        console.log(
